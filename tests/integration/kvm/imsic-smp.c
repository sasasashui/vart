...
typedef struct SharedState {
    ...
    _Atomic uint32_t received[VART_AIA_SMP_HARTS];
...
typedef struct TestContext {
    ...
    unsigned int exits[VART_AIA_SMP_HARTS];
} TestContext;

_Static_assert(offsetof(SharedState, ready) == VART_AIA_SMP_READY_OFFSET,
               "ready offset mismatch");
_Static_assert(offsetof(SharedState, received) ==
                   VART_AIA_SMP_RECEIVED_OFFSET,
               "received offset mismatch");
_Static_assert(offsetof(SharedState, error) == VART_AIA_SMP_ERROR_OFFSET,
                       ...
                       void *opaque)
{
    ...
    if (hart >= VART_AIA_SMP_HARTS || context->exits[hart]++ >= 4 ||
...
{
    VartVcpu vcpus[VART_AIA_SMP_HARTS];
    bool vcpu_created[VART_AIA_SMP_HARTS] = { false, false };
    ...
    for (i = 0; i < VART_AIA_SMP_HARTS; i++) {
    ...
    aia_created = true;
    ret = vart_riscv_aia_init_imsic(&aia, VART_AIA_SMP_HARTS,
    ...
    ret = vart_kvm_device_get_attr(&aia.device,
                                   ...
                                   &hart_bits);
    for (i = 0; ret == 0 && i < VART_AIA_SMP_HARTS; i++) {
    ...
    if (ret == 0) {
        ret = vart_riscv_aia_signal_msi(&aia, 1,
                                        VART_AIA_SMP_INTERRUPT_ID);
    }
    if (ret == 0) {
    ...
    if (ret == 0) {
        ret = vart_riscv_aia_signal_msi(&aia, 0,
                                        VART_AIA_SMP_INTERRUPT_ID);
    }
    if (ret == 0) {
...
out:
    ...
    for (i = VART_AIA_SMP_HARTS - 1; i >= 0; i--) {
        if (vcpu_created[i]) {
    ...
    if (ret < 0) {
        fprintf(stderr, "not ok - targeted SMP IMSIC: %s\n",
                strerror(-ret));
        return EXIT_FAILURE;
    }
    printf("ok - target an IMSIC interrupt to each vCPU\n");
    return EXIT_SUCCESS;
}
