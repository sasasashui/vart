...
int main(int argc, char **argv)
{
    ...
    if (vart_riscv_aia_connect_irq(NULL, &irq, INTERRUPT_SOURCE) != -EINVAL ||
        vart_riscv_aia_connect_irq(&aia, NULL,
                                   INTERRUPT_SOURCE) != -EINVAL ||
        vart_riscv_aia_connect_irq(&aia, &irq, 0) != -EINVAL ||
        vart_riscv_aia_connect_irq(&aia, &irq,
    ...
    ret = vart_test_device_init(&context.device, VART_VIRT_TEST_BASE,
                                NULL, NULL);
    if (ret == 0) {
        ret = vart_riscv_aia_connect_irq(&aia, &irq, INTERRUPT_SOURCE);
    }
    if (ret == 0) {
        vart_test_device_connect_irq(&context.device, &irq);
        ret = vart_address_space_add(
            &address_space, &context.device.region);
    }
    if (ret < 0) {
    ...
    vart_kvm_close(&kvm);
    alarm(0);
    printf("ok - deliver a device interrupt through KVM APLIC\n");
    return EXIT_SUCCESS;
...
fail_vm:
    vart_kvm_close(&kvm);
    fprintf(stderr, "not ok - device to APLIC interrupt: %d\n", ret);
    return EXIT_FAILURE;
}
